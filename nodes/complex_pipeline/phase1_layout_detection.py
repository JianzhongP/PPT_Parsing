...
class TypeRefinementEngine:
    ...
    TYPE_MAPPING = {
        ...
        "管线甘特图": "chart",
        "甘特图": "chart",
        "pharma_pipeline": "chart",
        "timeline": "chart",
        "process_diagram": "chart",
        "moa_pathway": "chart",
        "表格": "table",
        "纯图片": "image",
        "图片": "image",
...
选项：[数据图表, 医药管线图, 流程图, 表格, 纯图片, 公式]

判断依据：
- 数据图表：包含坐标轴、柱状/折线/饼图等数据可视化
- 医药管线图：药物/资产沿临床阶段（如 IND/Phase1/Phase2/Phase3/NDA）推进的时间线/泳道/甘特样式图；即使外观类似表格，也优先归为医药管线图
- 流程图：节点+箭头的步骤流转或机制路径
- 表格：以单元格读数为主的行列表，重点是逐格数据读取而非阶段推进关系
- 纯图片：照片、插图、示意图（无数据结构）
- 公式：数学公式、化学式
